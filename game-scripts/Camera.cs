...
public class Camera : NL_Script
{
  ...
  public void IdleBobbing()
  {
    ...
    if (NITELITE.Input.GetKeyPressed(Keys.W))
    {
      ...
      Vec3 lerpPos = LerpVec3(pos, InitialPos, dt * LerpValue);
    ...
    else
    {
      ...
      Vec3 NewPos = new Vec3((float)(Math.Sin(Timer * Math.PI * 2) * BobAmplitude * 0.5), (float)(Math.Cos(5 * Timer * Math.PI) * BobAmplitude), pos.z);

      Vec3 lerpPos = LerpVec3(pos, NewPos, dt * 2f);
      transform.SetPosition(lerpPos);
    }
  }
